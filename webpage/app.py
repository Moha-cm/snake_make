...
def scatter_plot():
    ...
    if request.method == "POST":
        ...
        filter_ensemble_version_id = filter_gene["ensembl_gene_id_version"].values[0]
        dataset = pd.merge(meta_data_df, raw_counts_df, left_on=common_columns, right_on=common_columns)
        
        filter_dataset = dataset[[plot_x_axis,filter_ensemble_version_id,color_options]]
        
        fig = px.bar(dataset, x=plot_x_axis, y=filter_ensemble_version_id)

        graphJSON = json.dumps(fig, cls=plotly.utils.PlotlyJSONEncoder)
        
        return render_template('scatter_plot.html', raw_counts=raw_counts_data, meta_data=meta_data_data, col_option=colnames_meta, exp_option=raw_counts_column, graphJSON=graphJSON)
        ...
        return render_template('scatter_plot.html', raw_counts=raw_counts_data, meta_data=meta_data_data, col_option=colnames_meta, exp_option=raw_counts_column)
